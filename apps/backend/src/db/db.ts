...
const dbPath = path.resolve(__dirname, config.DATABASE_URL.startsWith('./') ? config.DATABASE_URL : `../../${config.DATABASE_URL}`);
...
const db = new verboseSqlite3.Database(dbPath, (err) => {
  if (err) {
    ...
    db.run("PRAGMA foreign_keys = ON;", (pragmaErr) => {
      if (pragmaErr) {
        logger.error("Failed to enable PRAGMA foreign_keys:", pragmaErr.message);
      } else {
...
export const run = (sql: string, params: unknown[] = []): Promise<DBRunResult> => {
  return new Promise((resolve, reject) => {
    db.run(sql, params, function (err) { // Use function() to access this.lastID
      if (err) {
        logger.error(`SQL Error (run): ${sql} | Params: ${params} | Error: ${err.message}`);
        reject(err);
      } else {
...
export const get = <T,>(sql: string, params: unknown[] = []): Promise<T | undefined> => {
  return new Promise((resolve, reject) => {
    db.get(sql, params, (err, row: T) => {
      if (err) {
        logger.error(`SQL Error (get): ${sql} | Params: ${params} | Error: ${err.message}`);
        reject(err);
      } else {
        resolve(row);
      }
...
export const all = <T,>(sql: string, params: unknown[] = []): Promise<T[]> => {
  return new Promise((resolve, reject) => {
    db.all(sql, params, (err, rows: T[]) => {
      if (err) {
        logger.error(`SQL Error (all): ${sql} | Params: ${params} | Error: ${err.message}`);
        reject(err);
      } else {
        resolve(rows);
      }
