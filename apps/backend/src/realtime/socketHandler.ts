...
export const initSocketIO = (httpServer: HttpServer): SocketIOServer => {
  ...
  io.use((socket, next) => {
    ...
    if (token) {
      ...
      if (user) {
        ...
        logger.info(
          `Socket authenticated for user: ${user.name} (ID: ${user.id}) - Socket ID: ${socket.id}`
  ...
  io.on("connection", (socket: Socket) => {
    ...
    if (!user) {
      logger.error(
        `User somehow undefined on socket connection after auth middleware for socket ID: ${socket.id}. Disconnecting.`
      );
    ...
    logger.info(
      `User ${user.name} (Socket ID: ${socket.id}) joined room user:${user.id}`
    ); // Changed username to name
    ...
    socket.on("join:room", (roomName: string) => {
      if (typeof roomName === "string" && roomName.length > 0) {
        ...
        logger.info(
          `User ${user.name} (Socket ID: ${socket.id}) joined custom room: ${roomName}`
        ); // Changed username to name
        ...
        logger.warn(
          `User ${user.name} (Socket ID: ${socket.id}) attempted to join invalid room: ${roomName}`
        ); // Changed username to name
      }
    });
    ...
    socket.on("send:message", (message: RealtimeMessagePayload) => {
      ...
      logger.info(
        `User ${user.name} (Socket ID: ${socket.id}) sent a message via socket: ${message.content}`
      ); // Changed username to name
    ...
    socket.on("disconnect", (reason) => {
      logger.info(
        `User ${user.name} (Socket ID: ${socket.id}) disconnected. Reason: ${reason}`
      ); // Changed username to name
    });
  });
