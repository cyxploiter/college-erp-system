...
export const initSocketIO = (httpServer: HttpServer): SocketIOServer => {
  ...
  io.use((socket, next) => {
    ...
    if (token) {
      ...
      if (user) {
        ...
        logger.info(`Socket authenticated for user: ${user.name} (ID: ${user.id}) - Socket ID: ${socket.id}`); // Changed username to name
        return next();
      }
    }
  ...
  io.on('connection', (socket: Socket) => {
    ...
    logger.info(`User ${user.name} (Socket ID: ${socket.id}) connected.`); // Changed username to name

    // Join a room specific to the user ID for direct messages
    socket.join(`user:${user.id}`);
    logger.info(`User ${user.name} (Socket ID: ${socket.id}) joined room user:${user.id}`); // Changed username to name
    
    // Example: Send a welcome message to the connected client
    const welcomeMessage: RealtimeMessagePayload = {
        id: `welcome-${socket.id}`,
        content: `Welcome, ${user.name}! You are connected to the real-time notification service.`, // Changed username to name
    ...
    socket.on('join:room', (roomName: string) => {
      if(typeof roomName === 'string' && roomName.length > 0) {
        ...
        logger.info(`User ${user.name} (Socket ID: ${socket.id}) joined custom room: ${roomName}`); // Changed username to name
        ...
        logger.warn(`User ${user.name} (Socket ID: ${socket.id}) attempted to join invalid room: ${roomName}`); // Changed username to name
      }
    });
    ...
    socket.on('send:message', (message: RealtimeMessagePayload) => {
      ...
      logger.info(`User ${user.name} (Socket ID: ${socket.id}) sent a message via socket: ${message.content}`); // Changed username to name
      socket.broadcast.emit('receive:message', { ...message, sender: user.name }); // Changed username to name
    });

    socket.on('disconnect', (reason) => {
      logger.info(`User ${user.name} (Socket ID: ${socket.id}) disconnected. Reason: ${reason}`); // Changed username to name
    });
  });
