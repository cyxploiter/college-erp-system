...
import sectionRoutes from './routes/sectionRoutes'; // Added section routes
...
app.get('/api/health', (req, res) => {
  (res as any).status(200).json({ status: 'UP', timestamp: new Date().toISOString() });
});
...
app.use('/api/sections', sectionRoutes); // Added section routes
...
app.use((req, res) => {
  logger.warn(`404 Not Found: ${(req as any).method} ${(req as any).originalUrl}`);
  (res as any).status(404).json({ success: false, message: 'Resource not found.' });
});
