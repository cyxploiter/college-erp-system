...
export const createMessage = async (input: CreateMessageInput): Promise<Message> => {
  const { senderId, receiverId, subject, content, type, priority = 'Normal' } = input;
  ...
  const newMessageFromDb = await get<Message & { senderName?: string }>( 
    `SELECT m.*, s.name as senderName 
     ...
     WHERE m.id = ?`, [result.lastID] // result.lastID is for the messages.id (integer)
    );
  if (!newMessageFromDb) {
    logger.error(`Failed to retrieve newly created message with ID: ${result.lastID}`);
  ...
  if (newMessageFromDb.senderName) { 
    newMessage.sender = { name: newMessageFromDb.senderName, id: newMessageFromDb.senderId } as Partial<User> & { id?: string }; // Add senderId to sender object
  }
...
export const getReceivedMessagesForUser = async (userId: string): Promise<Message[]> => { // userId is string
  ...
  return messagesFromDb.map(msgFromDb => {
    ...
    if (msgFromDb.senderName) {
      msg.sender = { name: msgFromDb.senderName, id: msgFromDb.senderId } as Partial<User> & { id?: string };
    }
...
export const markMessageAsRead = async (messageId: number, userId: string): Promise<boolean> => { // userId is string
  logger.debug(`Marking message ID ${messageId} as read for user ID ${userId}`);
  // messages.receiverId is TEXT
  const message = await get<Message>('SELECT receiverId, type FROM messages WHERE id = ?', [messageId]);
  if (!message) {
  ...
  const result = await run('UPDATE messages SET isRead = TRUE WHERE id = ?', [messageId]);
