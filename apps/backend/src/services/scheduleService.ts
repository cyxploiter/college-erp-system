...
import { ScheduleItem, Section, Course, Semester, User, StudentEnrollment, UserRole } from '@college-erp/common';
...
const populateSectionDetails = async (sectionId: number): Promise<Section | null> => {
    const section = await get<Section & { // Type for raw DB result with aliased columns
        course_id: number, course_courseCode: string, course_courseName: string, course_credits: number,
        semester_id: number, semester_name: string, semester_year: number, semester_term: string, semester_startDate: string, semester_endDate: string,
        faculty_id?: string, faculty_name?: string, faculty_email?: string // faculty_id is now string
    }>(`
        SELECT 
            s.*,
    ...
    const populatedSection: Section = {
        ...
        faculty: (section as any).faculty_id ? { // faculty.id is now string
...
export const getUserSchedules = async (userId: string, userRole: UserRole): Promise<PopulatedScheduleItem[]> => { // userId is string
  ...
  if (userRole === 'student') {
    ...
    const sectionsTaught = await all<Section>('SELECT id FROM sections WHERE facultyUserId = ?', [userId]);
  ...
  if (sectionIds.length === 0) {
    logger.info(`No sections found for user ID: ${userId}. Returning empty schedule.`);
    return [];
  }
  ...
  logger.info(`Found ${populatedSchedules.length} schedule items for user ID: ${userId}`);
