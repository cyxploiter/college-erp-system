...
export const loginUser = async (
  ...
  userFromDb = await get<User>(
    "SELECT id, name, passwordHash, email, profilePictureUrl, departmentId FROM users WHERE id = ?",
    [identifier]
  );
  ...
  if (userFromDb) {
    ...
    logger.debug(
      `Identifier "${identifier}" matched a direct user ID: ${userId}`
    );
  } else {
    // If not a direct ID match, try as email
    logger.debug(
      `Identifier "${identifier}" not a direct user ID, checking as email.`
    );
    userFromDb = await get<User>(
      "SELECT id, name, passwordHash, email, profilePictureUrl, departmentId FROM users WHERE email = ?",
      [identifier]
    );
    if (userFromDb) {
      userId = userFromDb.id;
      logger.debug(
        `Identifier "${identifier}" (email) resolved to user ID: ${userId}`
      );
    } else {
      logger.warn(
        `Login attempt: Identifier "${identifier}" not found as ID or email.`
      );
    }
  }
  ...
  if (!userFromDb.passwordHash) {
    logger.warn(
      `Login failed: User record for ID ${userId} (from identifier "${identifier}") missing password hash.`
    );
    throw new HttpError("Invalid identifier or password.", 401);
  }
  ...
  if (!isPasswordValid) {
    logger.warn(
      `Login failed: Invalid password for user ${userFromDb.name} (ID: ${userFromDb.id}, Identifier: "${identifier}").`
    );
    throw new HttpError("Invalid identifier or password.", 401);
  }
  ...
  if (!actualRole) {
    logger.error(
      `Login failed: Role could not be determined for user ${userFromDb.name} (ID: ${userFromDb.id}). This indicates a data integrity issue.`
    );
    throw new HttpError(
  ...
  logger.info(
    `User ${userFromDb.name} (ID: ${userFromDb.id}, Role: ${actualRole}, Identifier: "${identifier}") logged in successfully.`
  );
